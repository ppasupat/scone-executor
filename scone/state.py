class SconeState(object):
    ...
    def from_raw_string(cls, raw_string):
        """Create a new SconeState from dataset string.
    ...
    def apply_action(self, action, stack):
        ...
        Returns:
            ...
            new_state (SconeState): State after the action is applied
            history_entry (tuple): An entry to be added to history
        """
        raise NotImplementedError

    def resolve_argument(self, argument):
        """Return a SconeObject that corresponds to the history argument
        but with the current properties.

        Args:
            argument (SconeObject)
        Returns:
            SconeObject
        """
        raise NotImplementedError
...
class SconeAlchemyObject(tuple, SconeObject):
    __slots__ = ()

    def __new__(self, position, chemicals):
        """Create a new SconeAlchemyObject.
        ...
        return tuple.__new__(SconeAlchemyObject, (position, chemicals, color))
...
class SconeAlchemyState(SconeState):
    ...
    @classmethod
    def from_raw_string(cls, raw_string):
        """Create a new SconeAlchemyState from dataset string.
        ...
        for raw_object in raw_string.split():
            ...
            objects.append(SconeAlchemyObject(
                int(raw_position),
    ...
    def apply_action(self, action, stack):
        if action == 'Pour':
            ...
            new_objects[target_pos - 1] = SconeAlchemyObject(
                    target_pos, target.chemicals + source.chemicals)
            new_objects[source_pos - 1] = SconeAlchemyObject(source_pos, '')
            ...
            if isinstance(drain_amount, str) and drain_amount[0] == 'X':
                ...
                drain_amount = target.amount * numer / denom
...
class SconeSceneObject(tuple, SconeObject):
    __slots__ = ()

    def __new__(self, position, shirt, hat, id_):
        """Create a new SconeSceneObject.
        ...
        """
        return tuple.__new__(SconeSceneObject, (position, shirt, hat, id_))
...
class SconeSceneState(SconeState):
    ...
    def __init__(self, objects, next_id):
        """Create a new SconeSceneState.

        Args:
            objects (list).
            next_id (int): The next available object ID.
        """
        SconeState.__init__(self, objects)
    ...
    @classmethod
    def from_raw_string(cls, raw_string):
        """Create a new SconeSceneState from dataset string.
        ...
        for raw_object in raw_string.split():
            ...
            if raw_colors != '__':
                objects.append(SconeSceneObject(
    ...
    def apply_action(self, action, stack):
        if action == 'Leave':
            ...
            for x in self._objects:
                if x.id_ == target1_id:
                    new_objects.append(SconeSceneObject(
    ...
    def resolve_argument(self, argument):
        ...
        return SconeSceneObject(0, argument.shirt, argument.hat, argument.id_)


################################
# Tangrams Domain

class SconeTangramsObject(tuple, SconeObject):
    __slots__ = ()

    def __new__(self, position, shape):
        """Create a new SconeTangramsObject.

        Args:
            position (int): Position of the tangram (starting with 1)
            shape (str): Shape ID.
        """
        return tuple.__new__(SconeTangramsObject, (position, shape))
...
class SconeTangramsState(SconeState):
    ...
    @classmethod
    def from_raw_string(cls, raw_string):
        """Create a new SconeTangramsState from dataset string.
        ...
        for raw_object in raw_string.split():
            ...
            objects.append(SconeTangramsObject(
    ...
    def apply_action(self, action, stack):
        if action == 'Add':
            ...
            new_tangram = SconeTangramsObject(new_pos, target_shape)
            ...
            for x in self._objects:
                ...
                if x.position < new_pos:
                    ...
                    new_objects.append(SconeTangramsObject(
    ...
    def resolve_argument(self, argument):
        ...
        for x in self._objects:
            if x.shape == argument.shape:
                return x
        # The object is not in the scene
        return SconeTangramsObject(0, argument.shape)


class SconeUndogramsState(SconeTangramsState):
    ...
    def apply_action(self, action, stack):
        new_state, command = SconeTangramsState.apply_action(self, action, stack)
        if action == 'Remove':
